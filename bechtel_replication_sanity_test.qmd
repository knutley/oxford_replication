...
*However, the replication data for figure 4, although it does not contain the original experiment responses, does contain sufficient data for selected individuals to reconstruct the response schedule – which will enable validation of the estimation code:*
...
estimate_sensitivies <- function(i_schedule) {
  ...
  #
  #o estimate d-sensitivity (the elasticity of redistribution behavior to disadvantageous inequality) we regress for each respondent the amount given on ΔPoorer using allobservations for which ΔPoorer ≥ 0 and multiply the estimated coefficient by -1. 
  
 reg_poor <- x.df %>%
      dplyr::filter(d_poorer>=0 & d_valid_choice) %>%
      stats::lm(data = ., formula = given ~ d_poorer) %>% 
      summary() %>% 
      broom::tidy()
 ...
 #To estimate a- sensitivity (the elasticity of redistribution behavior to advantageous inequality), we regress for eachrespondent the amount given on ΔRicher using all observations for which ΔRicher ≥ 0. 
  ...
  reg_rich <- x.df %>%
      ...
      stats::lm(data = ., formula = given ~ d_richer) %>% 
      summary() %>% 
      broom::tidy()
...
fig4.df %>% 
  dplyr::mutate(endowment=IA1, 
                ...
                given = -1*round(other  * shareTaken) ) %>%
  dplyr::select(`ID`,`endowment`,`other`,`given`) %>%
  tidyr::nest(sched=c(`endowment`,`other`,`given`)) -> sched.df
...
orig_extend.df %>% 
  ...
  dplyr::ungroup() %>%
  tidyr::unnest(est_sensitivities) -> orig_reestimates.df

View(orig_reestimates.df  %>% select(dplyr::contains("sensitivity"), `atype`, `dtype`))
```
...
**As it turns out the answers to the schedule is not a set of answers to repeated question, but a single answer to the prompt, and not possible to parse reliably automatically. Thus we generate an arbitrary synthetic schedule for test purposes.**
...
```{r}
#| label: experiment_clean_and_synthesize
...
# a beheviorally unrealistic, but not logically impossible schedule generator
gen_giving_schedule <- function( initial_e, initial_a ) {
  schedule.df <- tibble::tibble(
    endowment = c(15, 25, 50, 75, 85, 95)
  )
 ...
 schedule.df %<>% 
   dplyr::rowwise() %>%
   dplyr::mutate(amount =
                   initial_a + -1* sign(initial_e-endowment) * 
                   (sqrt(abs(initial_e-endowment))+5*runif(1,min=-1,max=1)),
                 amount = max(amount, endowment-100),
                 amount = min(amount, endowment)
                 ) %>%
  dplyr::ungroup()
...
t2m_synth.df <- t2m_std.df %>%
  dplyr::rowwise() %>%
  dplyr::mutate(schedule = list(gen_giving_schedule(endowment,amount))) %>%
  dplyr::ungroup()
```
...
```{r}
#| label: synthetic results

t2m_synth.df %>% 
  dplyr::rowwise() %>%
  dplyr::mutate(est_sensitivities = list(estimate_sensitivies(schedule))) %>%
  dplyr::ungroup() %>%
